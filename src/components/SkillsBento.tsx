 ...
 * ENGAGEMENT ROADMAP. This section is about HOW WE OPERATE day-to-day, so
 * none of it restates a capability, phase bullet, or outcome already said
 * elsewhere on the page.
 */
const modules = [
  {
    icon: Boxes,
    title: "An expert in AI. An expert in your business.",
  ...
  },
  {
    ...
    desc: "Done-for-you delivery. We execute the work, not just diagnose it and hand you a plan.",
    span: "col-span-2",
    anchor: false,
    tone: 23,
  },
 ...
 * (205, 40%); only the lightness (`tone`, in %) changes per tile, tonal
 * variation rather than hue variation, per the brief. This used to reuse
  ...
  return (
    <section className="py-20 md:py-28 bg-background">
      <div className="container mx-auto px-6">
        ...
        {/* The colored frame itself is a static backdrop, not an animated
            target — only the grid of tiles inside it gets the reveal
            treatment, so each tile gets its own staggered entrance rather
            than the whole frame fading in as one flat block. */}
        <div style={{ backgroundColor: "hsl(205 40% 17%)" }} className="rounded-[1.75rem] p-2 md:p-3">
          <RevealOnScroll className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3" stagger={0.1}>
            {modules.map((m) => {
              const Icon = m.icon;
              return (
                <div
